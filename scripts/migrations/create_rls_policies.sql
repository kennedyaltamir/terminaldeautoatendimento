...
-- Cria políticas de isolamento baseadas na variável de sessão app.current_company_id
...
-- 2. PRODUCTS
-- Nota: Products não tem company_id direto no schema reportado, mas deveria ter ou ser via join.
-- O relatório diz: Has company_id: False.
-- Se products não tem company_id, o RLS deve ser via Category (que tem company_id).
-- Policy via JOIN:
...
CREATE POLICY tenant_isolation_policy ON products
    ...
    FOR ALL
    TO public
    USING (category_id IN (SELECT id FROM categories WHERE company_id = nullif(current_setting('app.current_company_id', true), '')::uuid))
    WITH CHECK (category_id IN (SELECT id FROM categories WHERE company_id = nullif(current_setting('app.current_company_id', true), '')::uuid));
