...
# VERSION: 2.1.0 (Recovery Master)
# DESCRIPTION: Modelos financeiros consolidados: Carteira, Taxas e Transações.
...
class FinancialLedger(Base):
    """
    Livro Razão Soberano (L7).
    Garante a integridade financeira via encadeamento de hashes.
    """
    ...
    entry_type = Column(String(10), nullable=False) # CREDIT / DEBT
    amount = Column(BigInteger, nullable=False) # Em centavos
...
class CustomerWallet(Base):
    """
    Gestão de Cashback e Fidelidade do Cliente Final.
    """
...
class ServiceFeeLedger(Base):
    """
    Registro de Gorjetas e Taxas de Serviço do Staff.
    """
...
class DriverLedger(Base):
    """
    Extrato de ganhos e débitos do entregador.
    """
    ...
    order_id = Column(GUID(), ForeignKey("orders.id"), nullable=True)
    type = Column(String(50), nullable=False) # CREDIT / DEBT / PAYMENT
    amount = Column(Numeric(10, 2), nullable=False)
...
class PaymentTransaction(Base):
    """
    Log de transações com gateways externos (MP / Stripe).
    """
    __tablename__ = "payment_transactions"
    id = Column(GUID(), primary_key=True, default=uuid.uuid4)
    company_id = Column(GUID(), ForeignKey("companies.id"), nullable=False)
    order_id = Column(GUID(), ForeignKey("orders.id"), nullable=False)
    provider = Column(String(50), nullable=False) # mercadopago, stripe, efi
