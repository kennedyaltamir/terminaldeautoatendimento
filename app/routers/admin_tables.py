# DOMAIN: BACKEND
# LAST_MODIFIED: 2026-01-15 05:10:00
...
from app.models import (
    Company, Table, TableSession, Order, ServiceRequest, 
    OrderStatus, PaymentStatus, PaymentMethod, ServiceFeeLedger, Employee
)
...
def get_tables(
    ...
    """Lista todas as mesas da empresa."""
...
def update_table_positions(
    ...
    """Atualiza as coordenadas X/Y das mesas no mapa."""
    company_id = current_user.id if isinstance(current_user, Company) else current_user.company_id
...
def get_tables_dashboard(
    ...
    """Retorna o estado atual de todas as mesas para o painel administrativo."""
    company_id = current_user.id if isinstance(current_user, Company) else current_user.company_id
    ...
    for table in tables:
        ...
        if active_session:
            status = "occupied"
            # Calcula total pendente da sessão
...
async def open_table_session(
    ...
    """Abre uma nova sessão em uma mesa livre."""
...
async def pay_table_partial(
    ...
    """Processa um pagamento parcial de pedidos pendentes na mesa."""
    company = current_user if isinstance(current_user, Company) else current_user.company
    company_id = company.id
...
async def close_table_session(
    ...
    """Fecha a conta da mesa, processa gorjetas e encerra a sessão."""
    company = current_user if isinstance(current_user, Company) else current_user.company
    company_id = company.id
    slug = company.slug
    ...
    if data.custom_service_fee is not None:
        tip_amount = Decimal(str(data.custom_service_fee))
    else:
...
def update_session_name(
    ...
    """Altera o nome do cliente na sessão ativa."""
    company_id = current_user.id if isinstance(current_user, Company) else current_user.company_id
...
def get_session_details(
    ...
    """Retorna detalhes completos de uma sessão (pedidos e itens)."""
    company_id = current_user.id if isinstance(current_user, Company) else current_user.company_id
    session = db.query(TableSession).filter(TableSession.id == session_id, TableSession.company_id == company_id).first()
    if not session: 
        raise HTTPException(status_code=404, detail="Sessão não encontrada")
...
def transfer_table(
    ...
    """Transfere uma sessão de uma mesa para outra ou unifica contas."""
    company_id = current_user.id if isinstance(current_user, Company) else current_user.company_id
    ...
    if target_session:
        ...
        # Simples transferência de mesa física
