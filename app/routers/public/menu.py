# DOMAIN: BACKEND
# LAST_MODIFIED: 2026-01-23 10:50:00
...
def resolve_domain(host: str, db: Session = Depends(get_db)):
    """Resolve o slug da empresa baseado no host/domínio customizado."""
...
def get_menu(request: Request, company_slug: str, db: Session = Depends(get_db)):
    """Retorna o cardápio completo com isolamento RLS."""
    ...
    for cat in all_categories:
        if cat.availability_days and len(cat.availability_days) > 0:
...
def get_public_monitor_orders(company_slug: str, db: Session = Depends(get_db)):
    """Lista pedidos recentes para o telão do salão."""
    company = db.query(Company).filter(Company.slug == company_slug).first()
...
async def validate_kiosk_exit(
    ...
    FIX: Adicionado argumento 'request' para compatibilidade com o limiter.
    """
    company = db.query(Company).filter(Company.slug == company_slug).first()
    if not company:
...
def get_customer_wallet(
    ...
    """
    Consulta saldo de cashback do cliente.
    FIX: Adicionado argumento 'request' para compatibilidade com o limiter.
    """
    company = db.query(Company).filter(Company.slug == company_slug).first()
    if not company:
