...
export default function FiscalSection({ company }: FiscalSectionProps) {
  ...
  const {
    ...
    defaultValues: {
      name: company.name || "Minha Loja", // Garante que o nome satisfaça o schema
      cnpj: company.cnpj || "",
  ...
  const handleSearchCNPJ = async () => {
    ...
    try {
      ...
      // Remove formatação do CNPJ para o input
  ...
  const onSubmit = async (data: SettingsSchema) => {
    setLoading(true);
    try {
      // Limpa CNPJ para enviar apenas números
      ...
      window.location.reload(); // Recarrega para limpar estado de erro visual
    } catch (error: any) {
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 animate-in fade-in">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {/* Input Oculto para satisfazer o Schema */}
