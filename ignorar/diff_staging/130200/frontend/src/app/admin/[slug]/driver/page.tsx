...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  // Detecção de ambiente de teste para mockar comportamentos instáveis
  ...
  // 🔄 Sincronização Inteligente (FIXED: Loop Infinito Resolvido)
  // Se o backend diz que um pedido está 'delivering', assumimos ele como ativo se não houver outro
  // Removida dependência circular de activeDeliveryId
  useEffect(() => {
    // Se já temos um ativo, não faz nada
    if (activeDeliveryId !== null) return;

    const serverActive = orders.find(o => o.status === 'delivering');
    if (serverActive) {
        // Usa callback para garantir que só atualiza se realmente mudou
        setActiveDeliveryId(prev => {
            if (prev === serverActive.id) return prev;
            return serverActive.id;
        });
    }
  }, [orders]); // Dependência única: orders (activeDeliveryId removido para quebrar o loop)
  ...
  useEffect(() => {
    if (activeDelivery && typeof navigator !== "undefined" && "geolocation" in navigator) {
      // Em teste, não tentamos acessar GPS real para evitar permissões/timeout
      if (isTestEnv) return;
  ...
  useEffect(() => {
    ...
    if (activeDelivery) {
      // Mock total em ambiente de teste para evitar fetch externo e loops
      if (isTestEnv) {
      ...
      fetchRoute([driverLat, driverLng], [destLat, destLng])
        ...
        .catch(err => {
            // Silencia erro em teste se vazar
            if (!isTestEnv) console.error("Route error:", err);
        });
    }
