...
# Rota de compatibilidade para o padrão /admin/{slug}/tables do frontend
@router.get("/{slug}/tables", response_model=List[TableResponse], tags=["Admin - Tables"])
def get_tables_by_slug(
    slug: str,
    db: Session = Depends(get_db),
    current_user: any = Depends(get_current_user)
):
    ...
    user_company_id = current_user.id if isinstance(current_user, Company) else current_user.company_id
    if str(user_company_id) != str(company.id):
        raise HTTPException(status_code=403, detail="Acesso negado a esta empresa")

    return db.query(Table).filter(Table.company_id == company.id).order_by(Table.table_number).all()
