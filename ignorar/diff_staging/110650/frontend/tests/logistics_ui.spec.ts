...
test('Test 9: Driver Transitions to Map', async ({ page }) => {
    ...
    // Aguarda visibilidade com timeout estendido para suportar latência da API
    await expect(card).toBeVisible({ 
        timeout: 15000,
        message: "Falha: Nenhum pedido disponível para coleta. Verifique o Seed ou se outro teste consumiu os dados."
    });
    
    // 2. Prepara a captura da resposta de rede
    ...
    // 4. Aguarda a confirmação do backend
...
test('Test 10: Client Passive Tracking', async ({ page }) => {
    // Interceptação de rede para garantir conformidade arquitetural
    page.on('request', req => {
        const url = req.url();
    ...
    // Se não houver pedidos READY, tenta buscar o pedido que o Teste 9 acabou de colocar em rota
    ...
    if (await availableCard.isVisible()) {
        ...
        // Fallback: Se o Teste 9 deixou um pedido ativo, usamos ele
