...
export default function AdminMenuPage({ params }: { params: { slug: string } }) {
  ...
  const fetchMenu = async () => {
    try {
      ...
      setMenu(data);
      setExpandedCategories(data.categories.map((c: any) => c.id));
    } catch (error) {
      console.error(error);
  ...
  const handleSaveProduct = async () => {
    ...
    // Convert string input (e.g. "10,50") back to integer cents (1050)
  ...
  const handleAddOption = async () => {
    ...
    try {
      // Converte input string para centavos
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      {/* Header e Links - Mantidos iguais */}
              ...
              <div className="p-4 space-y-4 animate-in slide-in-from-top-2">
                {category.products.map((product) => (
                  <div key={product.id} className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { 
                            ...
                            setProdForm({ 
                                ...
                                price: centsToInput(product.price), // Conversão para Input
                    ...
                    {/* Lista de Grupos de Adicionais (Expandido) */}
      ...
      <Modal isOpen={isProdModalOpen} onClose={() => setIsProdModalOpen(false)} title={editingProduct ? "Editar Produto" : "Novo Produto"}>
         <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            />
            {/* ... Resto do Modal (Station, Estoque) Mantido ... */}
            <div>
