...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    fetchOrders();
    const interval = setInterval(fetchOrders, 30000); // Polling reduzido para 30s (WS faz o grosso)
    return () => clearInterval(interval);
  }, [fetchOrders]);

  // Sincroniza o estado com Normalização de Unicidade
  useWebSocket(slug, (data) => {
    if (data.type === "order_update") {
      setOrders(prev => prev.map(o => {
        // Se este pedido virou delivering, garante que nenhum outro esteja
        if (o.id === data.order_id) return { ...o, status: data.status };
  ...
  // Fonte de verdade única para o delivery ativo
  const activeDelivery = useMemo(() => orders.find(o => o.status === 'delivering'), [orders]);

  // GPS REAL: Propaga posição, mas NÃO dispara recálculo de rota
  useEffect(() => {
  ...
  }, [activeDelivery?.id]); // Só re-assina se o ID do pedido mudar

  // PERFORMANCE: Calcula a rota apenas 1x por pedido ou se a posição inicial for zerada
  useEffect(() => {
    if (activeDelivery) {
      // Prioriza coordenadas do pedido, fallback para mock de Pompéu
      const destLat = activeDelivery.table ? -19.22815 : -19.22815; // Placeholder p/ delivery_lat
      const destLng = activeDelivery.table ? -44.94195 : -44.94195;
      
      fetchRoute(driverPos, [destLat, destLng]).then(setRouteData);
    } else {
      setRouteData(null);
    }
  }, [activeDelivery?.id]); // Dependência apenas no ID. O driverPos NÃO dispara este effect.
  ...
  const handlePickup = async (orderId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // Normalização imediata: Seta este como único delivering
        setOrders(prev => prev.map(o => 
          o.id === orderId ? { ...o, status: 'delivering' } : (o.status === 'delivering' ? { ...o, status: 'ready' } : o)
        ));
      }
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 pb-24 font-sans">
      ...
      <header className="flex justify-between items-center mb-6 bg-slate-900 p-5 rounded-[2rem] border border-slate-800 shadow-2xl">
        <div className="flex items-center gap-4">
          <div className="bg-orange-600 p-3 rounded-2xl shadow-lg shadow-orange-900/20"><Bike size={28} /></div>
          <div>
            <h1 className="font-black text-xl tracking-tight">Painel do Entregador</h1>
        ...
        {activeDelivery ? (
          <div className="space-y-6 animate-in fade-in duration-500" data-testid="driver.delivery.active">
             <div className="bg-white text-slate-900 p-8 rounded-[2.5rem] shadow-2xl flex justify-between items-center border-b-8 border-orange-500">
                ...
                <div className="text-right">
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tempo Estimado</p>
             ...
             <div className="h-[500px] w-full" data-testid="driver.delivery.map">
                <TrackingMap 
                  driverPos={driverPos} 
          ...
          <section className="space-y-4 px-2">
            <h2 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] px-4">Coletas Pendentes</h2>
            ...
            {orders.filter(o => o.status === 'ready').length === 0 && (
               <div className="py-20 text-center text-slate-600 italic">Nenhuma coleta disponível no momento.</div>
            )}
          </section>
        )}
      </main>
