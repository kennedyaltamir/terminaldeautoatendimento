...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  // Ref para debounce imediato (mais rápido que state)
  const isSubmittingRef = useRef(false);

  // FIX: Removido isTransitioning da dependência para evitar loop de renderização
  // O estado de loading/transitioning deve ser controlado localmente ou via ref se necessário
  ...
  const availableOrders = useMemo(() => orders.filter(o => o.status === 'ready'), [orders]);
  ...
  const handlePickup = async (orderId: string) => {
    if (isTransitioning || isSubmittingRef.current) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Se já estiver despachado (idempotência), trata como sucesso
  ...
  // FIX: Removido useEffect que causava loop de renderização ao setar isTransitioning
  // O estado isTransitioning agora é controlado apenas pelo handlePickup e resetado em caso de erro.
  // Se activeDelivery for setado, a UI muda automaticamente, não precisamos resetar o loading state explicitamente via effect.
  useEffect(() => {
    if (activeDelivery) {
        setIsTransitioning(false);
    }
              ...
              availableOrders.map(order => (
                ...
                >
                  ...
                  <button 
                    ...
                    className="bg-orange-600 hover:bg-orange-500 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all disabled:opacity-50"
                  >
