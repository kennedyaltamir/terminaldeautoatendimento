...
async def dispatch_order(
    ...
    # LOCK 1: Busca o pedido com bloqueio de linha
    ...
    # IDEMPOTÊNCIA: Se o pedido já está em rota com ESTE motorista, retorna sucesso (200)
    if order.status == OrderStatus.DELIVERING and order.driver_id == driver_id:
        return {"message": "Rota já iniciada (Idempotent)", "status": "delivering"}

    # VALIDAÇÃO: Se já está em rota com OUTRO motorista
    if order.status == OrderStatus.DELIVERING:
        raise HTTPException(400, "Este pedido já foi coletado por outro entregador.")

    # VALIDAÇÃO: Status correto para coleta
    if order.status != OrderStatus.READY:
        raise HTTPException(400, "Pedido ainda não está pronto para entrega.")

    # LOCK 2: Verifica se o motorista já tem outra entrega ativa
    if driver_id:
        active_job = db.query(Order).filter(
