...
import { Bike, MapPin, Loader2, Compass, ArrowLeft, RefreshCw } from "lucide-react";
...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    fetchOrders();
    const interval = setInterval(fetchOrders, 15000); // Polling de segurança
    return () => clearInterval(interval);
  }, [fetchOrders]);

  // Sincroniza status e força atualização de lista em novos eventos
  useWebSocket(slug, (data) => {
    if (data.type === "new_order") {
      fetchOrders(); // Força recarregamento se um novo pedido entrar
    }
  ...
  const handlePickup = async (orderId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setOrders(prev => prev.map(o => 
          String(o.id) === String(orderId) ? { ...o, status: 'delivering' } : (o.status === 'delivering' ? { ...o, status: 'ready' } : o)
        ));
      }
      ...
      <main className="space-y-6">
        {activeDelivery ? (
          <div className="space-y-6 animate-in fade-in zoom-in duration-500" data-testid="driver.delivery.active">
             ...
             <button 
                onClick={() => setOrders(prev => prev.map(o => o.id === activeDelivery.id ? {...o, status: 'ready'} : o))}
                className="w-full py-4 bg-slate-800 text-slate-400 rounded-2xl font-bold uppercase text-xs flex items-center justify-center gap-2"
             >
          ...
          <section className="space-y-4 px-2">
            ...
            {orders.filter(o => o.status === 'ready').map(order => (
              <div 
                ...
                className="bg-slate-900 rounded-[2.5rem] p-8 border border-slate-800 shadow-xl flex justify-between items-center hover:border-orange-500/50 transition-all group"
              >
                <div className="flex-1 mr-4">
                  <h3 className="font-black text-lg tracking-tight group-hover:text-orange-500 transition-colors">{order.customer_name}</h3>
            ...
            {!loading && orders.filter(o => o.status === 'ready').length === 0 && (
               <div className="py-20 text-center text-slate-600 italic font-bold">Nenhum pedido pronto para coleta.</div>
            )}
          </section>
        )}
      </main>
