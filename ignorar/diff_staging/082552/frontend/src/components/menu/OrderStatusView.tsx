...
export default function OrderStatusView({ order, onNewOrder, primaryColor }: { order: Order, onNewOrder: () => void, primaryColor: string }) {
  ...
  // Carrega última posição conhecida do cache para evitar o modo "cego" (L6 Fallback)
  useEffect(() => {
    const cached = localStorage.getItem(`last_pos_${order.id}`);
    if (cached) setDriverPos(JSON.parse(cached));
  }, [order.id]);
  ...
  useWebSocket(slug, (data) => {
    if (data.order_id === order.id || data.payload?.order_id === order.id) {
      if (data.type === "order_update") {
        ...
        const newPos: [number, number] = [data.payload.lat, data.payload.lng];
        setDriverPos(newPos);
        localStorage.setItem(`last_pos_${order.id}`, JSON.stringify(newPos));
      }
    }
  });
  ...
  useEffect(() => {
    if (localStatus === 'delivering' && driverPos) {
       // OSRM ETA - Calculado de forma passiva no cliente para suavizar a UX
