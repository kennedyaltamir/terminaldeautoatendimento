# DOMAIN: DEVOPS_SCRIPTS
# LAST_MODIFIED: 2026-01-15 18:10:00
import subprocess
import sys
import os
from pathlib import Path

# ==============================================================================
# 🥾 QA BOOTSTRAPPER v1.0 (Windows & Unix)
# ==============================================================================
# Garante que todas as dependências de teste (Pytest, Playwright, Browsers) 
# estejam instaladas e prontas para uso.
# ==============================================================================

def run_cmd(cmd, cwd=None):
    print(f"🛠️  Executando: {cmd}")
    try:
        subprocess.run(cmd, shell=True, check=True, cwd=cwd)
        return True
    except subprocess.CalledProcessError:
        return False

def bootstrap():
    print("====================================================")
    print("📦 MESAFLOW QA DEPENDENCY BOOTSTRAP")
    print("====================================================")

    # 1. Python Deps (Venv check)
    print("\n[1/3] Verificando dependências Python...")
    run_cmd(f"{sys.executable} -m pip install pytest pytest-asyncio httpx --no-audit")

    # 2. Frontend Deps (Node.js)
    print("\n[2/3] Verificando dependências de Frontend...")
    frontend_path = Path("frontend")
    if not frontend_path.exists():
        print("❌ Erro: Pasta 'frontend' não encontrada.")
        return

    # Força a instalação do runner de teste
    run_cmd("npm install -D @playwright/test --no-audit", cwd="frontend")

    # 3. Playwright Browsers
    print("\n[3/3] Instalando binários dos navegadores (Chromium)...")
    run_cmd("npx playwright install chromium", cwd="frontend")

    print("\n✅ QA BOOTSTRAP CONCLUÍDO.")
    print("👉 Agora você pode rodar 'python scripts/validation/run_all_tests.py'.")

if __name__ == "__main__":
    bootstrap()
