...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  // Estado de loading específico para a ação de finalizar
  ...
  const activeDelivery = useMemo(() => {
    ...
    return orders.find(o => o.status === 'delivering' && !recentlyFinished.current.has(o.id)) || null;
  ...
  const handleFinish = async () => {
    ...
    isSubmittingRef.current = true;
    setIsFinishing(true); // Feedback visual imediato
    ...
    try {
        ...
        if (res.ok) {
            ...
            setOrders(prev => prev.map(o => 
                o.id === finishedId ? { ...o, status: 'delivered' } : o
            ));
            setActiveDeliveryId(null);
            fetchOrders();
        } else {
        ...
        {activeDelivery ? (
          <div className="space-y-6 animate-in fade-in duration-500" data-testid="driver.delivery.active">
             ...
             <button 
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed text-white py-6 rounded-[2rem] font-black text-xl uppercase tracking-widest shadow-2xl shadow-emerald-900/30 flex items-center justify-center gap-3 active:scale-95 transition-all"
             >
