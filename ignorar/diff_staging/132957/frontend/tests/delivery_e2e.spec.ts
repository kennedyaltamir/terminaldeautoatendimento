...
test.describe('Logistics E2E: Happy Path', () => {
  test('Fluxo Completo: Coleta -> Entrega -> Conclusão', async ({ page }) => {
    ...
    // 7. Validar Sucesso (Contrato de Estado Persistente)
    // REMOVIDO: Validação de Toast (Efêmero/Flaky em Headless)
    // MANTIDO: Validação de Estado (Painel deve sumir)
    // O teste valida o contrato do sistema (mudança de estado), não o feedback visual transitório.
