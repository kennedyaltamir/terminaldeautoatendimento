...
# LAST_MODIFIED: 2026-01-15 15:50:00
import asyncio
import json
import sys
import os
...
async def run_l8_simulation():
    print(f"🧬 MESAFLOW L8.7 AUTONOMOUS QUALITY SYSTEM | Build: {BUILD_ID}")
    ...
    with SimulationTransaction(API_URL, SLUG, token) as tx:
        async with async_playwright() as p:
            ...
            # 🛡️ L8 HARDENING: Aguarda a transição visual para o modo ativo no driver
            await expect(driver_page.get_by_test_id("driver.delivery.active")).to_be_visible(timeout=10000)
            ...
            # 🛡️ L8 HARDENING: Usa Test ID para o botão de finalização
