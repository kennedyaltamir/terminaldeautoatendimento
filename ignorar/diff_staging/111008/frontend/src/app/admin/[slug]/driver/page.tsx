...
// Importação dinâmica para evitar erros de referência ao 'window' no servidor (Leaflet)
const TrackingMap = dynamic(() => import("@/components/ui/TrackingMap"), { 
  ssr: false,
  loading: () => <div className="w-full h-full bg-slate-900 animate-pulse flex items-center justify-center text-slate-500 font-bold">INICIALIZANDO MAPA...</div>
});
...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  const fetchOrders = useCallback(async () => {
    if (isTransitioning) return; // Bloqueia overwrite durante rito de despacho
    try {
  ...
  useEffect(() => {
    if (activeDelivery && typeof navigator !== "undefined" && "geolocation" in navigator) {
      watchId.current = navigator.geolocation.watchPosition(
        ...
        () => toast.error("GPS Offline: Verifique as permissões."),
  ...
  const handlePickup = async (orderId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // RESILIÊNCIA L6: Atualização local imediata para evitar dependência do WebSocket/Redis
        setOrders(prev => prev.map(o => 
      ...
      toast.error("Erro de conexão ao iniciar rota"); 
      setIsTransitioning(false);
    }
  };

  // Libera o lock de transição quando o estado 'delivering' é confirmado no array de ordens
  useEffect(() => {
          ...
          <section className="space-y-4 px-2">
            ...
            {orders.filter(o => o.status === 'ready').length === 0 ? (
              <div className="py-20 text-center text-slate-600">
                <p className="font-bold">Nenhum pedido pronto para coleta.</p>
              </div>
            ) : (
