...
# LAST_MODIFIED: 2026-01-15 11:55:00
import asyncio
import requests
import json
import sys
...
# ==============================================================================
# 🛵 REAL-TIME DELIVERY & GPS SIMULATION (L6.21 - POMPÉU/MG CINEMATIC)
# ==============================================================================
...
async def setup_order():
    ...
    customer_name = f"CLIENTE_POMPEU_{unique_id}"
...
async def run_simulation():
    ...
    async with async_playwright() as p:
        ...
        # FIX: Strict mode fix using exact: True to target the stepper label specifically
        await expect(customer_page.get_by_text("Pronto", exact=True)).to_be_visible(timeout=20000)
        ...
        print("⏳ [5/5] Validando ROTA DINÂMICA no Cliente...")
        await customer_page.bring_to_front()
        await expect(customer_page.get_by_text("Em Rota")).to_be_visible(timeout=15000)

        # Simula movimento GPS fluido (Pompéu, MG)
        print("📍 Iniciando percurso: Rua João Machado -> Rua Padre João Porto...")
        start_lat, start_lng = -19.22448, -44.93548
        end_lat, end_lng = -19.22815, -44.94195
        steps = 20
        ...
        print("\n\n🏆 SUCESSO: Entrega em Pompéu/MG concluída e validada visualmente!")
        
        # Manter aberto para observação se necessário
