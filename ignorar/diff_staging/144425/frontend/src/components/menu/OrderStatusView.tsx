...
"use client";
import { 
  Clock, ChefHat, CheckCircle2, MapPin, Plus, Bike, PartyPopper 
} from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col pb-10 font-sans" data-testid="customer.order-view">
      ...
      <div className="p-6 space-y-6 max-w-md mx-auto w-full">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100" data-testid="customer.order.stepper">
          ...
          {/* RENDERIZAÇÃO CONDICIONAL DE ESTADO */}
          {localStatus === 'delivered' ? (
            <div className="py-12 text-center space-y-6 animate-in zoom-in duration-500" data-testid="customer.order.success-view">
               <div className="w-24 h-24 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto shadow-lg">
                  <PartyPopper size={48} />
               </div>
               <div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight">Pedido entregue!</h2>
                  <p className="text-slate-500 font-bold text-sm mt-2">Esperamos que você aproveite sua refeição. Bom apetite! 🍽️</p>
               </div>
            </div>
