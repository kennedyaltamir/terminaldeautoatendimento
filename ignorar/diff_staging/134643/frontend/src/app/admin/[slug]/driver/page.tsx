...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  // 🛡️ Proteção contra Stale Data: IDs finalizados recentemente são ignorados pelo auto-select
  ...
  useEffect(() => {
    ...
    // Encontra pedido 'delivering' que NÃO foi finalizado recentemente nesta sessão
    ...
    try {
      ...
      if (res.ok) {
        ...
        setOrders(prev => {
            ...
            // Mantém consistência visual se tivermos um ativo localmente
  ...
  const handleFinish = async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // 1. Marca como finalizado para evitar re-seleção por stale data
