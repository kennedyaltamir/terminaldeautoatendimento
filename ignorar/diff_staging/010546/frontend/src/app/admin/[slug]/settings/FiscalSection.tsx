...
export default function FiscalSection({ company }: FiscalSectionProps) {
  ...
  const {
    ...
    defaultValues: {
      name: company.name || "Minha Loja", // Preenche para evitar erro de validação do Zod (min 3 chars)
      cnpj: company.cnpj || "",
  ...
  const handleSearchCNPJ = async () => {
    ...
    try {
      ...
      // Atualiza o valor limpando a formatação para passar no Zod
  ...
  const onError = (errors: any) => {
    console.log("Falha na validação do formulário:", errors);
  ...
  const onSubmit = async (data: SettingsSchema) => {
    setLoading(true);
    try {
      // Garante que o CNPJ vá apenas como números
      ...
      // Pequeno delay e reload para limpar estados visuais
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 animate-in fade-in">
      {/* Input invisível para satisfazer a obrigatoriedade do 'name' no Schema */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {errors.cnpj && <p className="text-red-400 text-[10px] font-bold mt-1 flex items-center gap-1"><AlertCircle size={10}/> {errors.cnpj.message}</p>}
            </div>
            ...
            <div>
              ...
              />
              {errors.inscricao_estadual && <p className="text-red-400 text-[10px] font-bold mt-1 flex items-center gap-1"><AlertCircle size={10}/> {errors.inscricao_estadual.message}</p>}
              <p className="text-[10px] text-gray-500 mt-1">Obrigatório. Use "ISENTO" se não tiver.</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-gray-700 pb-2">
                ...
                <a href="https://focusnfe.com.br" target="_blank" className="text-[10px] text-blue-400 hover:text-blue-300 flex items-center gap-1">
            ...
            <div>
              ...
              <div className="relative">
                <input
                    ...
                    type="password"
                    className={`w-full bg-gray-900 border rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all font-mono text-sm pr-24 ${errors.fiscal_token ? 'border-red-500 bg-red-900/10' : 'border-gray-600'}`}
              ...
              {errors.fiscal_token && <p className="text-red-400 text-[10px] font-bold mt-1 flex items-center gap-1"><AlertCircle size={10}/> {errors.fiscal_token.message}</p>}
            </div>
