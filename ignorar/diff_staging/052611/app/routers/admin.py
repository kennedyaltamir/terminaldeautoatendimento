# DOMAIN: BACKEND
# LAST_MODIFIED: 2026-01-15 08:45:00
...
async def update_order_payment(order_id: UUID, payment_update: OrderPaymentUpdate, db: Session = Depends(get_db), current_user: any = Depends(get_current_user)):
    """Permite ao staff confirmar o pagamento de um pedido manualmente."""
    company_id = current_user.id if hasattr(current_user, 'owner_email') else current_user.company_id
    ...
    db.commit()
    return {"message": f"Status de pagamento atualizado para {payment_update.payment_status}"}
