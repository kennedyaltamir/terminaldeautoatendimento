...
export default function LeadCapture() {
  ...
  useEffect(() => {
    // Trigger: Aparece após 15 segundos ou se o usuário tentar sair (exit intent simulado)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulação de integração com CRM
    await new Promise(r => setTimeout(r, 1500));
    
    setLoading(false);
    setSubmitted(true);
    toast.success("Guia enviado! Verifique sua caixa de entrada.");
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
          ...
          >
            <button 
              onClick={handleClose} 
              className="absolute top-6 right-6 text-slate-400 hover:text-slate-600 dark:hover:text-white z-20 bg-slate-100 dark:bg-slate-800 p-2 rounded-full transition-all hover:rotate-90"
            >
              <X size={20} />
            </button>

            <div className="flex flex-col md:flex-row min-h-[450px]">
              {/* Lado Visual (Aspiracional) */}
                ...
                <div className="absolute bottom-10 left-8 right-8">
                  <div className="bg-white/10 backdrop-blur-lg p-5 rounded-2xl border border-white/20 shadow-xl">
                    <p className="text-white font-medium text-sm italic leading-relaxed">
                      "O guia de engenharia de cardápio nos ajudou a aumentar o ticket médio em 18% sem mudar um único ingrediente."
                    </p>
                ...
                {submitted ? (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 dark:text-slate-400 mt-2">
                        O link para o <b>Guia de Eficiência 2026</b> foi enviado para seu e-mail profissional.
                      </p>
                    </div>
                  </motion.div>
                ) : (
                  <>
                    ...
                    <h3 className="text-4xl font-black text-slate-900 dark:text-white leading-[1.1] mb-4 tracking-tighter">
                      Venda mais com <br/><span className="text-orange-600">menos esforço.</span>
                    </h3>
                    ...
                    <p className="text-[10px] text-center text-slate-400 mt-8 uppercase font-bold tracking-widest">
                      🔒 Privacidade garantida. Odiamos spam tanto quanto você.
                    </p>
                  </>
                )}
              </div>
