...
async def create_order(
    ...
    if order_data.order_type == "dine_in":
        if not order_data.table_id or not order_data.qr_token:
            raise HTTPException(status_code=400, detail="Mesa e Token são obrigatórios")
        table = db.query(Table).filter(Table.id == order_data.table_id, Table.company_id == company.id).first()
        if not table:
            raise HTTPException(status_code=404, detail="Mesa não encontrada")
        
        session = db.query(TableSession).filter(TableSession.table_id == table.id, TableSession.is_active == True).first()
        if not session:
    ...
    for item in order_data.items:
        ...
        db_items.append(OrderItem(product_id=product.id, quantity=item.quantity, unit_price=product.price, notes=item.notes))
    ...
    await manager.broadcast({"type": "new_order", "order_id": str(new_order.id)}, company.slug)
    return new_order
