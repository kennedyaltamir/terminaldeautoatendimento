...
// Importação dinâmica para evitar erros de hidratação com Leaflet
...
export default function DriverPage({ params }: { params: { slug: string } }) {
    ...
    if (activeDelivery && typeof navigator !== "undefined" && "geolocation" in navigator) {
      watchId.current = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          // Propaga localização para o backend
  ...
  const handlePickup = async (orderId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // RESILIÊNCIA L6: Atualização local imediata para evitar dependência do WebSocket
        setOrders(prev => prev.map(o => 
  ...
  // Libera o lock de transição quando o estado 'delivering' é confirmado
  useEffect(() => {
