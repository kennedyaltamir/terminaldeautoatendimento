...
// Carregamento dinâmico do Leaflet para evitar erros de SSR (Server-Side Rendering)
...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  const fetchOrders = useCallback(async () => {
    // Bloqueia a atualização via polling se uma transição de despacho estiver em curso
  ...
  useEffect(() => {
    fetchOrders();
    // Polling de segurança (10s) caso o WebSocket ou Redis falhem no ambiente
  ...
  // Identifica se há uma entrega ativa para este motorista
  const activeDelivery = useMemo(() => 
    orders.find(o => o.status === 'delivering'), 
  [orders]);

  // Filtra pedidos prontos para coleta
  const availableOrders = useMemo(() => 
    orders.filter(o => o.status === 'ready'), 
  [orders]);

  // Monitoramento de GPS Nativo
  useEffect(() => {
    if (activeDelivery && typeof navigator !== "undefined" && "geolocation" in navigator) {
      watchId.current = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          // Propaga localização para o backend (rastreio do cliente)
        ...
        () => toast.error("GPS Offline: Verifique as permissões do navegador."),
  ...
  // Cálculo de Rota OSRM (Open Source Routing Machine)
  useEffect(() => {
    if (activeDelivery) {
  ...
  const handlePickup = async (orderId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // RESILIÊNCIA L6: Atualização local imediata para redundância ao WebSocket
        setOrders(prev => prev.map(o => 
  ...
  // Libera o lock de transição quando o estado 'delivering' é detectado no array
  useEffect(() => {
