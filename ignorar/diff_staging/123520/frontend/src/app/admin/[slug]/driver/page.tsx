...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Evita re-render desnecessário se os dados forem iguais (Otimização básica)
        setOrders(prev => {
  ...
  // FIX: Decomposição de dependências para evitar loop infinito de renderização
  // O array driverPos muda de referência a cada update do GPS, causando loop no useEffect
  ...
  // Reset transition state when active delivery is set
  useEffect(() => {
    if (activeDelivery) {
        setIsTransitioning(false);
    }
      ...
      <header className="flex justify-between items-center mb-6 bg-slate-900 p-5 rounded-[2rem] border border-slate-800 shadow-2xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-black uppercase tracking-widest">MesaFlow Logistics</p>
          </div>
        </div>
