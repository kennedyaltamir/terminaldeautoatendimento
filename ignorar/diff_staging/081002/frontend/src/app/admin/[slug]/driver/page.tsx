...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  // Sincroniza o estado da lista local com o WebSocket
  useWebSocket(slug, (data) => {
    if (data.type === "order_update") {
      // Atualiza o status do pedido na lista local sem precisar de um novo fetch
  ...
  // Filtra o pedido que está sendo entregue AGORA
  ...
  }, [!!activeDelivery]); // Re-executa se o ID do pedido ativo mudar

  useEffect(() => {
    if (activeDelivery) {
      // Destino Pompéu (Mock de destino do cliente)
  ...
  const handlePickup = async (orderId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // Força a transição imediata no estado local para abrir o mapa
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 pb-24 font-sans">
      ...
      <main className="space-y-6">
        {/* Se houver um pedido em entrega, mostra o MAPA DE ROTA */}
        {activeDelivery ? (
          <div className="space-y-6 animate-in fade-in zoom-in duration-500" data-testid="delivery.active-section">
             <div className="bg-white text-slate-900 p-8 rounded-[2.5rem] shadow-2xl flex justify-between items-center border-b-8 border-orange-500">
                ...
                <div className="text-right">
                   ...
                   <p className="text-3xl font-black text-orange-600" data-testid="delivery.eta-display">
          ...
          /* Se não, mostra a LISTA DE PEDIDOS DISPONÍVEIS */
          <section className="space-y-4 px-2 animate-in slide-in-from-bottom-5">
            ...
            {orders.filter(o => o.status === 'ready').length === 0 && (
               <div className="py-20 text-center text-slate-600 italic font-bold">Aguardando novos pedidos...</div>
            )}
          </section>
        )}
      </main>
