...
async def run_l8_simulation():
    print(f"🧬 MESAFLOW L8.5 AUTONOMOUS QUALITY SYSTEM | Build: {BUILD_ID}")
    ...
    with SimulationTransaction(API_URL, SLUG, token) as tx:
        async with async_playwright() as p:
            ...
            # 5. LOGISTICS & TELEMETRY (L8.5 Hardened)
            ...
            # 🛡️ L9 PREVIEW: Injeção de Telemetria como Gatilho de UI
            requests.post(f"{API_URL}/admin/delivery/orders/{order_id}/location", 
                         headers=headers, json={"lat": START_COORD[0], "lng": START_COORD[1]})
            
            # Assert Semântico: O mapa deve ser montado reativamente ao dado de GPS
            await expect(client_page.get_by_test_id("customer.order.map")).to_be_visible(
                timeout=15000,
                message="FALHA ARQUITETURAL: O componente de mapa não foi montado após o recebimento da telemetria."
            )

            # Rota com Jitter e Velocidade Variável
            steps = 5
            ...
            print("[5/5] Selagem de Evidência L8.5...")
