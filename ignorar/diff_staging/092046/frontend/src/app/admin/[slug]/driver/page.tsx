...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  useWebSocket(slug, (data) => {
    if (data.type === "order_update") {
      setOrders(prev => prev.map(o => String(o.id) === String(data.order_id) ? { ...o, status: data.status } : o));
    }
  });
            ...
            {orders.filter(o => o.status === 'ready').map(order => (
              ...
              >
                <div className="flex-1">
                  <h3 className="font-black text-lg tracking-tight" data-testid="driver.delivery.order.customer-name">{order.customer_name}</h3>
