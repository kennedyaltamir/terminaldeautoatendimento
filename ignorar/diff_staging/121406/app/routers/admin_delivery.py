...
async def dispatch_order(
    ...
    company_id = getattr(current_user, "company_id", getattr(current_user, "id", None))
    # Se for Admin (Company), driver_id é None. Se for Employee, é o ID dele.
    driver_id = current_user.id if isinstance(current_user, Employee) else None

    # LOCK: Busca com bloqueio de linha
    ...
    # IDEMPOTÊNCIA: Se já está com ESTE motorista (ou Admin/None), retorna sucesso
    # Isso corrige o erro 400 em cliques duplos
    ...
    # Validação de Motorista Ocupado (Apenas se for Driver real)
    if driver_id:
