# DOMAIN: BACKEND
# LAST_MODIFIED: 2026-01-15 17:30:00
import math
...
async def dispatch_order(
    ...
    """Inicia a entrega com LOCK TRANSACIONAL."""
    company_id = getattr(current_user, "company_id", getattr(current_user, "id", None))
    
    # 🛡️ TYPE SAFETY: Apenas Employee pode ser driver_id (Integer)
    # Donos (Company) têm ID UUID e não podem ser driver_id no banco.
    driver_id = current_user.id if isinstance(current_user, Employee) else None
    
    # LOCK 1: Driver concurrency
    if driver_id:
