...
export default function DriverPage({ params }: { params: { slug: string } }) {
  ...
  // GPS REAL: Monitora posição do dispositivo e propaga via POST
  useEffect(() => {
    if (activeDelivery && typeof navigator !== "undefined" && "geolocation" in navigator) {
      console.log("🛰️ GPS Real Conectado.");
  ...
  // Recalcula rota/ETA a cada atualização de posição do motorista
  useEffect(() => {
    if (activeDelivery) {
      // Destino Padrão (Pompéu, MG)
        ...
        {activeDelivery ? (
          ...
          <section className="space-y-4 px-2">
            ...
            {orders.filter(o => o.status === 'ready').map(order => (
              <div key={order.id} className="bg-slate-900 rounded-[2.5rem] p-8 border border-slate-800 shadow-xl flex justify-between items-center hover:border-orange-500/50 transition-all">
