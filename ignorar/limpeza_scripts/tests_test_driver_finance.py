...
def test_driver_finance_flow():
    ...
    # 4. Verificar Saldo via API (Deve ser 5000 centavos)
    res_balance = client.get(f"/api/admin/logistics/drivers/{driver_id}/balance", headers=headers)
    assert res_balance.status_code == 200
    # A API retorna em centavos (int)
    assert res_balance.json()["current_debt"] == 5000

    # --- FASE 2: PAGAMENTO DA DÍVIDA ---

    # 5. Pagar R$ 50 (Settlement) - Payload em centavos
    payload = {"amount": 5000, "description": "Acerto do dia"}
    ...
    assert res_balance_final.json()["current_debt"] == 0
