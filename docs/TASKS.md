...
- [x] **Build Nativo:** APK v1.0.1 gerado com correções semânticas.
- [x] **Missão 14A:** Endurecimento de Auth e decodificação resiliente de JWT.
- [x] **Bugfix:** Correção de chaves inválidas no SecureStore.
- [x] **Auditoria:** Criado script `master_alignment_check.py` para validação global.
...
- [x] **Build Nativo:** APK v1.0.1 gerado com correções semânticas.
- [x] **Missão 14A:** Endurecimento de Auth e decodificação resiliente de JWT.
- [x] **Auditoria Mestre:** Execução do `master_alignment_check.py` com 100% de sucesso.
- [x] **Resiliência Windows:** Patch de codificação UTF-8 aplicado em todos os scripts de auditoria.
...
## ✅ Concluído Recentemente (Fase 12 - Lançamento & Hardening)
- [x] **Build Nativo:** APK v1.0.1 gerado com correções semânticas.
- [x] **Ambiente Local:** Configuração de variáveis de ambiente para build nativo no Windows.
- [x] **Script de Diagnóstico:** Criado `check_mobile_env.py` para validação de infraestrutura.

---

## 🚀 Próximas Prioridades (Fase 12 - Campo & Hardware)

### 1. [Mobile] Missão 36: Primeiro Build Local (Bypass EAS) 🏗️
**Complexidade:** Média | **Impacto:** Produtividade
*   **O que fazer:** Executar `npx expo run:android` com sucesso após configuração do JDK/SDK.

### 2. [Mobile] Missão 37: Homologação de Impressão em Campo 🖨️
**Complexidade:** Alta | **Impacto:** Operacional
*   **O que fazer:** Testar a comunicação Bluetooth com uma impressora térmica real.
# 📋 Backlog Mestre de Tarefas: MesaFlow
...
### 🏗️ Core & Arquitetura
- [x] **Setup Inicial:** FastAPI + Next.js 14 + PostgreSQL + Docker.
- [x] **Multi-tenancy:** Isolamento lógico de dados por `company_id`.
- [x] **Autenticação:** JWT com suporte a múltiplos perfis (Dono, Gerente, Garçom, Cozinha).
- [x] **Segurança:** Rate Limiting, Sanitização de Inputs, Auditoria de Logs.
...
- [x] **App do Entregador:** Interface simplificada para motoboys.
- [x] **Rastreamento GPS:** Envio de localização em tempo real via WebSocket.
- [x] **Proof of Delivery (POD):** Confirmação de entrega via código de segurança.
- [x] **Gestão de Frota:** Cadastro e atribuição de entregadores.
...
**5. Cardápio Multilíngue Automático**
Detecção do idioma do navegador do cliente para exibir nomes e descrições traduzidas automaticamente (via Google Translate API ou banco de traduções), essencial para áreas turísticas e hotéis.
...
**17. Testes E2E (Cypress/Playwright)**
Criação de robôs que simulam um cliente real fazendo um pedido a cada deploy. Garante que o fluxo crítico de "Pedir -> Pagar -> Cozinha Receber" nunca quebre.
