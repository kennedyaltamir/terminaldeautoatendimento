  ...
  const fetchMetrics = async () => {
    ...
    try {
      ...
      if (period === "today") {
        ...
        const past = new Date();
        past.setDate(now.getDate() - 6); // Últimos 7 dias (incluindo hoje)
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-2xl shadow-lg p-6">
            <h2 className="text-xl font-bold text-white mb-6">Evolução de Vendas</h2>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={metrics?.sales_chart || []}>
                  ...
                  <YAxis stroke="#9ca3af" tick={{fontSize: 12}} tickLine={false} axisLine={false} tickFormatter={(value) => `R$${value}`} />
                  <Tooltip 
              ...
              {!metrics || metrics.top_products.length === 0 ? (
                ...
                <div className="space-y-4">
                  {metrics.top_products.map((product, index) => (
                    <div key={index} className="flex items-center justify-between bg-gray-900/50 p-4 rounded-xl border border-gray-700/50">
                      ...
                      <span className="bg-orange-600/20 text-orange-500 px-4 py-1 rounded-full text-sm font-bold">
