    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 p-4 md:p-6 font-sans">
      ...
      <div className="print:hidden">
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 border-b border-gray-800 pb-4 gap-4">
            ...
            <div className="flex bg-gray-800 p-1.5 rounded-xl overflow-x-auto max-w-full no-scrollbar border border-gray-700">
              <button onClick={() => handleTabChange('all')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'all' ? 'bg-gray-700 text-white shadow-md ring-1 ring-gray-600' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'}`}>
                <Layers size={16} /> Todos
              </button>
              <button onClick={() => handleTabChange('kitchen')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'kitchen' ? 'bg-orange-600 text-white shadow-md ring-1 ring-orange-500' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'}`}>
                <Utensils size={16} /> Cozinha
              </button>
              <button onClick={() => handleTabChange('bar')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'bar' ? 'bg-purple-600 text-white shadow-md ring-1 ring-purple-500' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'}`}>
                <Wine size={16} /> Bar
              </button>
              <button onClick={() => handleTabChange('dessert')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'dessert' ? 'bg-pink-600 text-white shadow-md ring-1 ring-pink-500' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'}`}>
                <IceCream size={16} /> Sobremesa
              </button>
            </div>
                    ...
                    <div className="flex justify-between items-start mb-3">
                        ...
                        {order.order_type === 'delivery' ? (
                            <div className="flex items-center gap-2 text-blue-400">
                                <Bike size={24}/>
                                <div><h2 className="text-xl font-black leading-none">Delivery</h2><p className="text-xs font-bold text-gray-400 mt-0.5">#{order.id.slice(0,4)}</p></div>
                            </div>
                        ) : (
                            <div><h2 className="text-2xl font-black leading-none text-white">Mesa {order.table?.table_number || "?"}</h2><p className="text-xs font-bold text-gray-500 mt-1">#{order.id.slice(0,4)} • {order.customer_name || "Cliente"}</p></div>
                        )}
                        </div>
                        <OrderTimer createdAt={order.created_at} />
                    </div>
                    <div className="flex justify-between items-center">
                       <span className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${order.status === 'pending' ? 'bg-green-900/30 text-green-400 border border-green-900/50' : 'bg-amber-900/30 text-amber-400 border border-amber-900/50'}`}>{order.status === 'pending' ? 'Novo Pedido' : 'Em Preparo'}</span>
                       <button onClick={() => handlePrint(order)} className="p-1.5 bg-gray-700 hover:bg-gray-600 rounded text-gray-400 hover:text-white transition-colors"><Printer size={14}/></button>
                    </div>
                </div>
                <div className="p-4 flex-1 overflow-y-auto max-h-[350px] bg-gray-800/30"><ul className="space-y-3">{renderItems(order.items)}</ul></div>
                <div className="p-4 bg-gray-900 border-t border-gray-800 mt-auto space-y-3">
                    {hasMixedItems(order) && <div className="flex items-center gap-2 text-[10px] text-yellow-500 bg-yellow-900/20 p-2 rounded border border-yellow-900/30"><AlertTriangle size={12} /><span>Atenção: Pedido contém itens de outra praça.</span></div>}
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col"><span className="text-lg font-black text-white">R$ {Number(order.total_amount).toFixed(2)}</span><span className="text-[10px] font-bold text-gray-500 uppercase tracking-wide">{order.payment_method}</span></div>
                        {order.payment_status === 'paid' ? <span className="text-green-500 flex items-center gap-1 text-xs font-bold bg-green-900/20 px-2 py-1 rounded border border-green-900/30"><CheckCircle2 size={14}/> PAGO</span> : <button onClick={() => handleConfirmPayment(order.id)} className="bg-gray-700 text-gray-300 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 hover:bg-green-600 hover:text-white transition-all border border-gray-600 hover:border-green-500"><DollarSign size={12}/> Confirmar</button>}
                    </div>
                    <button onClick={() => handleAdvanceStatus(order.id, order.status)} className={`w-full py-3.5 rounded-xl font-bold text-white shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 ${order.status === 'pending' ? 'bg-blue-600 hover:bg-blue-500 shadow-blue-900/20' : 'bg-green-600 hover:bg-green-500 shadow-green-900/20'}`}>{order.status === 'pending' ? <>Iniciar Preparo <ArrowRightCircle size={20} /></> : <>Finalizar Pedido <CheckCircle2 size={20} /></>}</button>
                </div>
                </div>
            ))}
            </div>
      ...
      <Modal isOpen={isRecallOpen} onClose={() => setIsRecallOpen(false)} title="Histórico Recente (Recall)">
        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
            {recentOrders.length === 0 ? <p className="text-gray-500 text-center py-8">Nenhum pedido finalizado recentemente.</p> : recentOrders.map(order => (
                <div key={order.id} className="bg-gray-800 p-4 rounded-xl border border-gray-700 flex justify-between items-center hover:border-gray-600 transition-colors">
                    <div><p className="font-bold text-white text-lg">{order.order_type === 'delivery' ? 'Delivery' : `Mesa ${order.table?.table_number}`} <span className="text-gray-500 font-normal text-sm ml-2 font-mono">#{order.id.slice(0,6)}</span></p><p className="text-xs text-gray-400 mt-1">{new Date(order.created_at).toLocaleTimeString()} • {order.items.length} itens • {order.customer_name}</p></div>
                    <button onClick={() => handleRestoreOrder(order.id)} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors shadow-lg shadow-blue-900/20"><Undo2 size={16} /> Restaurar</button>
                </div>
            ))}
        </div>
      </Modal>
      ...
      {printingOrder && (
        <div className="hidden print:block bg-white text-black p-4 w-[80mm] font-mono text-sm">
          <div className="text-center border-b border-dashed border-black pb-2 mb-2"><h2 className="text-lg font-bold uppercase">MesaFlow KDS</h2><p>Pedido: #{printingOrder.id.toString().slice(0, 8)}</p><p>{new Date(printingOrder.created_at).toLocaleString()}</p></div>
          <div className="mb-2">{printingOrder.order_type === 'delivery' ? <><p className="text-xl font-bold">DELIVERY</p><p>CLIENTE: {printingOrder.customer_name}</p><p>TEL: {printingOrder.customer_phone}</p><p className="mt-1 font-bold">ENDEREÇO:</p><p>{printingOrder.delivery_address}</p></> : <><p className="text-xl font-bold">MESA: {printingOrder.table?.table_number}</p><p>CLIENTE: {printingOrder.customer_name || "Não informado"}</p></>}</div>
          <div className="border-b border-dashed border-black pb-2 mb-2"><p className="font-bold mb-1">ITENS:</p>{printingOrder.items.map((item, i) => (<div key={i} className="mb-2"><div className="flex justify-between"><span>{item.quantity}x {item.product.name}</span></div>{item.selected_options?.map((o, j) => (<p key={j} className="ml-4 text-xs">+ {o.name}</p>))}{item.notes && <p className="ml-4 text-xs italic">Obs: {item.notes}</p>}</div>))}</div>
          <div className="flex justify-between font-bold text-lg"><span>TOTAL:</span><span>R$ {Number(printingOrder.total_amount).toFixed(2)}</span></div>
          <div className="mt-4 text-center text-xs border-t border-dashed border-black pt-2"><p>Pagamento: {printingOrder.payment_method.toUpperCase()}</p><p>Status: {printingOrder.payment_status.toUpperCase()}</p></div>
        </div>
      )}
