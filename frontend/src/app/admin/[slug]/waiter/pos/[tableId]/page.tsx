...
import { db } from "@/lib/db"; // Importar DB Local
...
export default function WaiterPOSPage({ params }: { params: { slug: string, tableId: string } }) {
  ...
  useEffect(() => {
    ...
    // Se estiver offline, tenta carregar do cache do navegador (se implementado service worker)
    // ou falha graciosamente. O menu deveria estar em cache.
    
    Promise.all([
      getMenu(slug).catch(() => null), // Falha silenciosa se offline
    ...
    checkTableStatus(slug, parseInt(tableId), "admin-override")
      .then(async (status) => {
      ...
      .catch(() => {
        // Se falhar ao checar mesa (offline), assume estado neutro ou avisa
        console.log("Offline: Não foi possível verificar status da mesa");
      });
  }, [slug, tableId]);
  ...
  const handleOpenTable = async () => {
    ...
    try {
      ...
      setIsTableOpen(true);
      // Recarregar página pode não ser ideal offline, melhor atualizar estado
  ...
  const handleSendOrder = async () => {
    ...
    try {
      ...
      // Fallback Offline
      console.log("Falha no envio, salvando localmente...", e);
      ...
      toast.warning(
        <div className="flex flex-col">
          ...
          <span className="text-xs">Pedido salvo no dispositivo. Será enviado quando a conexão voltar.</span>
        </div>,
  ...
  const handlePrintBill = () => {
    ...
    const consolidatedOrder: any = {
        id: `MESA-${tableId}`,
        created_at: new Date().toISOString(),
        table: { table_number: parseInt(tableId) },
        customer_name: customerName,
        order_type: 'dine_in',
        payment_method: 'A PAGAR',
        payment_status: 'pending',
        total_amount: sessionOrders.reduce((acc, o) => acc + Number(o.total_amount), 0),
  ...
  const sessionTotal = sessionOrders.reduce((acc, o) => acc + Number(o.total_amount), 0);
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div className="bg-gray-900 text-white p-4 shadow-md shrink-0 flex items-center gap-4 print:hidden">
        ...
        <div className="flex-1">
          <h1 className="font-bold text-lg">{terms.table} {tableId}</h1>
          {isTableOpen ? (
        ...
        <div className="flex gap-2">
            {isTableOpen && (
                <>
                  <button onClick={() => setIsTransferOpen(true)} className="bg-orange-600 p-2 rounded-full hover:bg-orange-500 text-white" title="Transferir Mesa">
                      <ArrowRightLeft size={20} />
                  </button>
                  <button onClick={() => setIsPaymentOpen(true)} className="bg-green-600 p-2 rounded-full hover:bg-green-500 text-white shadow-lg" title="Fechar Conta">
                      <CreditCard size={20} />
                  </button>
                  <button onClick={() => setIsAuditOpen(true)} className="bg-gray-700 p-2 rounded-full hover:bg-gray-600 text-blue-400" title="Ver Comanda">
                      <Eye size={20} />
                  </button>
                  <button onClick={handlePrintBill} className="bg-gray-700 p-2 rounded-full hover:bg-gray-600" title="Imprimir Parcial">
                      <Printer size={20} />
                  </button>
                </>
            )}
      ...
      {!isTableOpen && (
        <div className="bg-orange-100 p-4 border-b border-orange-200 flex gap-2 items-center print:hidden">
          ...
          <button onClick={handleOpenTable} className="bg-orange-600 text-white px-4 py-2 rounded font-bold text-sm">Abrir</button>
        </div>
      )}

      <div className="p-2 bg-white border-b border-gray-200 print:hidden">
        <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
            <input type="text" placeholder="Buscar por nome ou código..." className="w-full bg-gray-100 border-none rounded-lg pl-10 pr-4 py-3 text-sm outline-none" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>

      {/* ATALHOS RÁPIDOS (MAIS VENDIDOS) */}
      {topProducts.length > 0 && (
        <div className="bg-gray-50 border-b border-gray-200 p-2 overflow-x-auto no-scrollbar shrink-0 print:hidden">
          <p className="text-[10px] font-bold text-gray-400 uppercase mb-1 px-1 flex items-center gap-1"><Star size={10}/> Mais Vendidos</p>
          <div className="flex gap-2">
            {topProducts.map(p => (
              <button 
                key={p.id} 
      ...
      <div className="bg-white border-b border-gray-200 overflow-x-auto no-scrollbar shrink-0 print:hidden">
        <div className="flex p-2 gap-2">
          <button onClick={() => setActiveCategory(-1)} className={`whitespace-nowrap px-4 py-3 rounded-lg text-sm font-bold ${activeCategory === -1 ? 'bg-orange-600 text-white' : 'bg-orange-50 text-orange-700'}`}><Zap size={14} /> Rápidos</button>
      ...
      <div className="flex-1 overflow-y-auto p-2 grid grid-cols-2 gap-2 content-start print:hidden">
      ...
      <div className="bg-white border-t border-gray-200 p-4 safe-area-bottom shadow-[0_-4px_10px_rgba(0,0,0,0.1)] print:hidden">
        ...
        <div className="flex gap-3">
          <button onClick={() => setIsCartOpen(true)} className="flex-1 bg-gray-200 text-gray-800 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2"><ShoppingBag size={20} /> Ver Pedido</button>
          <button onClick={handleSendOrder} disabled={items.length === 0} className="flex-[2] bg-green-600 text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 disabled:opacity-50 shadow-lg shadow-green-200"><ChefHat size={20} /> Enviar</button>
        </div>
      </div>

      {isCartOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 flex justify-end print:hidden">
      ...
      {sessionId && (
        <>
            ...
            <PaymentModal isOpen={isPaymentOpen} onClose={() => setIsPaymentOpen(false)} tableId={parseInt(tableId)} tableName={`${terms.table} ${tableId}`} totalAmount={sessionTotal} onSuccess={() => router.push(`/admin/${slug}/waiter`)} />
            <TransferModal 
              isOpen={isTransferOpen} 
              onClose={() => setIsTransferOpen(false)} 
              fromTableId={parseInt(tableId)} 
              fromTableName={`${terms.table} ${tableId}`}
              slug={slug}
              onSuccess={() => router.push(`/admin/${slug}/waiter`)}
            />
        </>
      )}
