  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div className="bg-gray-900 text-white p-4 shadow-md shrink-0 flex items-center gap-4 print:hidden">
        ...
        <div className="flex gap-2">{isTableOpen && (<><button onClick={() => setIsTransferOpen(true)} className="bg-orange-600 p-2 rounded-full hover:bg-orange-500 text-white" title="Transferir Mesa"><ArrowRightLeft size={20} /></button><button onClick={() => setIsPaymentOpen(true)} className="bg-green-600 p-2 rounded-full hover:bg-green-500 text-white shadow-lg" title="Fechar Conta"><CreditCard size={20} /></button><button onClick={() => setIsAuditOpen(true)} className="bg-gray-700 p-2 rounded-full hover:bg-gray-600 text-blue-400" title="Ver Comanda"><Eye size={20} /></button><button onClick={handlePrintBill} className="bg-gray-700 p-2 rounded-full hover:bg-gray-600" title="Imprimir Parcial"><Printer size={20} /></button></>)}</div>
      </div>
      {!isTableOpen && <div className="bg-orange-100 p-4 border-b border-orange-200 flex gap-2 items-center print:hidden"><User size={20} className="text-orange-600" /><input type="text" placeholder="Nome do Cliente" className="flex-1 bg-white border border-orange-300 rounded px-3 py-2 text-sm outline-none" value={customerName} onChange={e => setCustomerName(e.target.value)} /><button onClick={handleOpenTable} className="bg-orange-600 text-white px-4 py-2 rounded font-bold text-sm">Abrir</button></div>}
      ...
      {sessionId && (<><BillAuditModal isOpen={isAuditOpen} onClose={() => setIsAuditOpen(false)} sessionId={sessionId} tableName={`${terms.table} ${tableId}`} /><PaymentModal isOpen={isPaymentOpen} onClose={() => setIsPaymentOpen(false)} tableId={parseInt(tableId)} tableName={`${terms.table} ${tableId}`} totalAmount={sessionTotal} onSuccess={() => router.push(`/admin/${slug}/waiter`)} /><TransferModal isOpen={isTransferOpen} onClose={() => setIsTransferOpen(false)} fromTableId={parseInt(tableId)} fromTableName={`${terms.table} ${tableId}`} slug={slug} onSuccess={() => router.push(`/admin/${slug}/waiter`)} /></>)}
      <ProductModal product={selectedProduct} isOpen={!!selectedProduct} onClose={() => setSelectedProduct(null)} onConfirm={(qty, notes, opts) => { if(selectedProduct) addToCart(selectedProduct, qty, notes, opts); setSelectedProduct(null); }} primaryColor="#ea580c" />
    </div>
  );
}