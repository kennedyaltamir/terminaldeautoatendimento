...
export default function AdminMenuPage({ params }: { params: { slug: string } }) {
  ...
  const handleSaveProduct = async () => {
    ...
    if (isNaN(priceFloat)) return toast.error("Preço inválido");
    ...
    <div className="space-y-8 pb-20">
      <div className="bg-gradient-to-r from-gray-800 to-gray-900 p-4 rounded-xl border border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4 shadow-lg">
        ...
        <div className="flex gap-2">
          <button onClick={copyPublicLink} className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors"><Copy size={14} /> Copiar</button>
      ...
      <Modal isOpen={isCatModalOpen} onClose={() => setIsCatModalOpen(false)} title="Nova Categoria">
        <div className="space-y-4">
            ...
            <button onClick={handleCreateCategory} className="w-full bg-orange-600 text-white py-3 rounded-lg font-bold hover:bg-orange-700 transition-colors">Criar Categoria</button>
        </div>
      </Modal>
      ...
      <Modal isOpen={isProdModalOpen} onClose={() => setIsProdModalOpen(false)} title={editingProduct ? "Editar Produto" : "Novo Produto"}>
         <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <input type="number" className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 text-white outline-none focus:ring-2 focus:ring-orange-500" placeholder="Preço (R$)" value={prodForm.price} onChange={e => setProdForm({...prodForm, price: e.target.value})} />
            ...
            <button onClick={handleSaveProduct} className="w-full bg-orange-600 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-orange-700 transition-colors"><Save size={20} /> Salvar Produto</button>
         </div>
      </Modal>
      ...
      <Modal isOpen={isGroupModalOpen} onClose={() => setIsGroupModalOpen(false)} title="Novo Grupo de Adicionais">
        <div className="space-y-4">
          ...
          <button onClick={handleAddGroup} className="w-full bg-orange-600 py-3 rounded-lg font-bold text-white hover:bg-orange-700">Criar Grupo</button>
        </div>
      </Modal>
      ...
      <Modal isOpen={isOptModalOpen} onClose={() => setIsOptModalOpen(false)} title="Nova Opção">
        <div className="space-y-4">
          ...
          <input type="number" className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 text-white outline-none focus:ring-2 focus:ring-orange-500" placeholder="Preço Adicional (0 para grátis)" value={optForm.price} onChange={e => setOptForm({...optForm, price: e.target.value})} />
          <button onClick={handleAddOption} className="w-full bg-orange-600 py-3 rounded-lg font-bold text-white hover:bg-orange-700">Adicionar Opção</button>
        </div>
      </Modal>
