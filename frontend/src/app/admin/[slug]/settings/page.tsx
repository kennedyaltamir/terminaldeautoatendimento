...
import { Save, Loader2, Store, Smartphone, CreditCard, Image as ImageIcon, Trash2, AlertTriangle, Zap, Bug, Bike } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      ...
      <div className="flex border-b border-gray-700 overflow-x-auto no-scrollbar">
        {[
          ...
          { id: "finance", label: "Financeiro & Delivery", icon: CreditCard },
          { id: "billing", label: "Assinatura", icon: Zap }
        ].map((tab) => (
          <button 
            key={tab.id} 
            onClick={() => setActiveTab(tab.id as any)} 
            className={`px-6 py-3 font-medium text-sm transition-colors border-b-2 flex items-center gap-2 whitespace-nowrap ${activeTab === tab.id ? "border-orange-500 text-orange-500" : "border-transparent text-gray-400 hover:text-white"}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
              ...
              {activeTab === "finance" && (
                <div className="space-y-6 animate-in fade-in">
                  ...
                  <div className="border-t border-gray-700 pt-6 space-y-4">
                    <h3 className="text-white font-bold">Meios de Pagamento</h3>
          ...
          <div className="bg-red-900/10 border border-red-900/30 rounded-xl p-6 space-y-4">
            ...
            <button onClick={() => { throw new Error("Teste Sentry Frontend: Erro Simulado pelo Usuário"); }} className="w-full text-red-400 hover:text-red-300 text-sm font-bold flex items-center justify-center gap-2 hover:bg-red-900/20 px-4 py-3 rounded-lg transition-colors border border-red-900/30">
              <Bug size={16} /> Simular Erro (Teste Sentry)
            </button>
            <button onClick={() => toast.error("Funcionalidade desativada na demo.")} className="w-full text-red-400 hover:text-red-300 text-sm font-bold flex items-center justify-center gap-2 hover:bg-red-900/20 px-4 py-3 rounded-lg transition-colors">
