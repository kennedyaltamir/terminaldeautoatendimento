...
function MenuContent({ slug }: { slug: string }) {
  ...
  const [walletBalance, setWalletBalance] = useState(0); // Em centavos? A API de wallet deve retornar centavos
  ...
  const { items, addToCart, updateCartItem, removeFromCart, clearCart, total } = useCart();
    ...
    if (customerPhone.length >= 8) {
      const timer = setTimeout(() => {
        getWallet(slug, customerPhone).then(data => {
          setWalletBalance(data.balance); // Centavos
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (res.valid) {
        ...
        setCouponMessage(`Desconto de ${formatCurrency(res.discount_amount)} aplicado!`);
      } else {
        setCouponDiscount(0);
        setAppliedCouponId(null);
    ...
    try {
      ...
      if (smartPosType && paymentMethod === 'card') {
        const intentUrl = generatePaymentIntent({
          scheme: smartPosType,
          amount: (total - couponDiscount) / 100, // SmartPOS espera Reais
          type: 'credit',
          orderId: order.id
        });
                ...
                {category.products.map((product: Product) => {
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <p className="font-bold" style={{ color: primaryColor }}>{formatCurrency(product.price)}</p>
          ...
          <div className="bg-white w-full max-w-md p-6 rounded-xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-4">
                ...
                <div key={idx} className="flex justify-between py-3 border-b last:border-0 group">
                    ...
                    <div className="text-right">
                        <p className="text-sm font-bold text-gray-900">{formatCurrency(((Number(item.product.price) + item.selectedOptions.reduce((a,b)=>a+Number(b.price),0)) * item.quantity))}</p>
            ...
            <div className="mt-4 px-4">
              ...
              <div className="flex justify-between text-xl font-black text-gray-900 border-t pt-2">
                <span>Total</span>
                <span>{formatCurrency(finalTotalDisplay)}</span>
              </div>
            </div>
