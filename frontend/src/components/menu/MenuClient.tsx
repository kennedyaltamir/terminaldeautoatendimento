...
export default function MenuClient({ slug }: { slug: string }) {
        ...
        if (tableId) {
          ...
          try {
            ...
            if (statusData.status === 'active' && statusData.session_token) {
              ...
              const session = await getTableSession(statusData.session_token);
            ...
            if (err.status === 403) {
              console.warn("Sessão expirada ou QR Code inválido. Purgando cache local...");
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-32 font-sans selection:bg-orange-500 selection:text-white">
      {isKiosk ? (
        <KioskHeader 
          ...
          >
            <div ref={cartBtnRef}>
              <button
                ...
                data-testid="cart-button" // 🛡️ FIX: Adicionado para o teste forense
              >
